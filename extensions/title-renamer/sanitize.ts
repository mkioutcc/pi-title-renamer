...
function stripWrapping(value: string): string {
	...
	while (changed) {
		...
		for (const [start, end] of pairs) {
			if (output.startsWith(start) && output.endsWith(end) && output.length >= start.length + end.length) {
...
function cleanCandidate(candidate: string): string {
	let output = candidate.replace(ANSI_ESCAPE_PATTERN, "").replace(CONTROL_PATTERN, "").trim();
